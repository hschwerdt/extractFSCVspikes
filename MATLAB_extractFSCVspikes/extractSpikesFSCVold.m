...
samplerate=30e3;           %DEFAULT sample rate for neuralynx data, 
...
if fractfoundfscv<.01
    ...
    %could be for next section of code 
                %DON"T USE, get average of signals later (just w 60 hz)
                %TEST figure, mean signal after FSCV interpolation, example channel
                %{
                curax=ax{5};
                title(curax,'Mean Signal After FSCV Interpolation')
                hold(curax,'on')
                ylabel(curax,'Voltage (V)')
                xlabel(curax,'Time (s)')
                sampleIDs=samplerate*90*60:samplerate*90*60+samplerate*1;    %Window 30 min + 1 s
                plot(curax,TS(sampleIDs)-TS(1),samplesmean(sampleIDs));
                plot(curax,TS(sampleIDs)-TS(1),samplesinterp(sampleIDs),'k');
                legend(curax,{'Original Signal','After FSCV Interpolation'})
                %}
...
pdfSave([temppath 'extractSpikesfig3session127_trial190-3'], [15 11.5], gcf);
